...
int_* null_function(int_* mass, int_ max)
{
...
void total_comander()
{
	...
	cout << "[#] \tCombined operation (A*B)/B;" << endl;
	cout << "[d] \tDistributivity testing (A+B)*C=C*(A+B)=A*C+B*C;" << endl;
	cout << "[c] \t Complexyty testing;" << endl;
...
int_* constant_null(int_* mass,int_ size){return null_function(mass, size);}

int_* constant_one(int_* mass, int_ size){null_function(mass, size);mass[0] = 1;return mass;}
...
void long_division(int_* mass_1, int_* dividor, int_* quotien, int_*remainder, int_ system_base, int_ max)
{
	...
	while ((long_compare(remainder, dividor, max) == 0) || (long_compare(remainder, dividor, max) == 1))
	{
		...
		null_function(array_for_substraction, max);
...
int_* long_power(int_* lower, int_* upper, int_ down, int_ up, int_* result, int_ size_result, int_ system_base)
{
	...
	null_function(_temp, size_result);
	null_function(temp_, size_result);
	...
	for (int_ i = 0; i < up; i++)
	{
		...
		if (upper[i] == 1)
		{
			...
			null_function(mass_, _down);
			copy_array(mass_, result, _down);
			null_function(result, _down);

			if (j == 1) { null_function(mass_, _down); mass_[0] = 1; j = 0; }
...
void test_distributivity(int_* mass_1, int_* mass_2, int_* mass_3, int_ old_size, int_ new_size, int_ size_result, int_ system_base)
{
	...
	null_function(_mass_1, size_result);
	null_function(_mass_2, size_result);
	null_function(_mass_3, size_result);
	null_function(mass_, size_result);
	null_function(_mass, size_result);
	...
	null_function(mass, size_result + 1);
...
{
	int_* remainder = new int_[max]; null_function(remainder, max);
	int_* quotient = new int_[max]; null_function(quotient, max);
	...
	int_* temp = new int_[2*max+1]; null_function(temp, 2*max+1);
	...
	int_* result = new int_[max]; null_function(result, max);
...
{
	int_* mass_ = new int_[2 * size + 1]; null_function(mass_, 2 * size + 1); copy_array(mass_, _mass, size);

	int_* module_ = new int_[size]; null_function(module_, size); copy_array(module_, module, size);

	int_* result = new int_[2 * size + 1]; null_function(result, 2 * size + 1);
...
int_* coefitient_barrett_(int_* module_number,int_* coefitient_barrett, int_ size, int_ system_base) 
{
	...
	int_* quotient = new int_[size_coeficient]; null_function(quotient, size_coeficient);
	int_* remainder = new int_[size_coeficient]; null_function(remainder, size_coeficient);

	int_* _module = new int_[size_coeficient]; null_function(_module, size_coeficient); copy_array(_module, module_number,size_number(module_number,size));
...
int_* algorithm_barrett_(int_* long_number, int_* module_number, int_* coefitient_barrett, int_* result, int_ size, int_ system_base, int_ max_for_coeficient)
{
	...
	int_* quotient_ = new int_[max];           null_function(quotient_, max);
	int_* quotient = new int_[max];            null_function(quotient, max);
	int_* remainder = new int_[max];           null_function(remainder, max);

	int_* long_number_ = new int[max];          null_function(long_number_, max);          copy_array(long_number_, long_number, size);
	int_* module_number_ = new int[max];        null_function(module_number_, max);        copy_array(module_number_, module_number, size);
	int_* coefitient_barrett_ = new int[max];   null_function(coefitient_barrett_, max);   copy_array(coefitient_barrett_, coefitient_barrett, size_number(coefitient_barrett, max_for_coeficient));

	copy_array(quotient_, function_kill_digit(long_number_, max, size_number(module_number_, max) - 1), max);

	int_* quitient_temp = new int_[2 * max + 1];null_function(quitient_temp, 2 * max + 1);
	...
	int_* temp_= new int_[2 * max + 1]; null_function(temp_, 2 * max + 1);
	...
	int_* _long_number = new int_[max];  null_function(_long_number, max);   copy_array(_long_number, long_number, size);
	...
	{
		int_* temp = new int_[max]; null_function(temp, max);
...
int_* add_mod_reduction(int_* first, int_* second, int_* mod, int_* coefitient_barrett, int_* result, int_ system_base, int_ max,int_ max_for_coeficient)
{
	...
	int_* first_ = new int_[max];  null_function(first_, max); 
	int_* second_ = new int_[max]; null_function(second_, max);
	...
	int_* mass_ = new int_[max+1]; 	null_function(mass_, max+1);     mass_ = long_add(first_, second_, mass_, max, system_base);

	int_* mod_ = new int_[max + 1]; null_function(mod_, max + 1); copy_array(mod_, mod, max);

	int_* result_ = new int_[max + 1]; null_function(result_, max + 1);
...
{
	int_* mass_ = new int_[max]; null_function(mass_, max);
	...
	{
		int_* _mass = new int_[max]; null_function(_mass, max);
...
int_* multiplication_mod_reduction(int_* first, int_* second, int_* mod, int_* coefitient_barrett, int_* result, int_ system_base, int_ max, int_ max_for_coeficient)
{
	...
	int_* mass_ = new int_[2*max+1]; null_function(mass_, 2*max+1);

	long_mul(first_, second_, mass_, system_base, 2*max+1);

	int_ * mod_ = new int_[2 * max + 1]; null_function(mod_, 2 * max + 1); copy_array(mod_, mod, max);

	int_ * result_ = new int_[2 * max + 1]; null_function(result_, 2 * max + 1); 
...
int_* elevation_mod_reduction(int_* lower, int_* upper, int_* mod, int_* coefitient_barrett, int_* result, int_ down, int_ up, int_ size_result, int_ max_for_coeficient,  int_ system_base)
{
	...
	int_* lower_ = new int_[down];    null_function(lower_, down);

	barrett_reduction_(lower, mod, coefitient_barrett, lower_, down, system_base, max_for_coeficient);

	int_* _temp = new int_[down];     null_function(_temp, down);    _temp = copy_array(_temp, lower_, down);
	int_* temp_ = new int_[down];     null_function(temp_, down);    temp_ = copy_array(temp_, lower_, down);

	int_* mod_ = new int_[2 * down + 1];  null_function(mod_, 2 * down + 1);  copy_array(mod_, mod, down);
	...
	for (int_ i = 0; i < up; i++)
	{
		...
		if (upper[i] == 1)
		{
			...
			int_* mass_ = new int_[down]; null_function(mass_, down); copy_array(mass_, result, down); null_function(result, down);

			if (j == 1) { null_function(mass_, down); mass_[0] = 1; j = 0; }

			int_* result_ = new int_[2 * down + 1];

			null_function(result_, 2 * down + 1);
		...
		if (i != up - 1) // we don`t make no need step in the end
		{
			...
			int_* temp = new int_[2*down+1]; null_function(temp, 2 * down+1);

			long_mul(_temp, temp_, temp, system_base, 2*down+1);

			int_* result_ = new int_[ down]; null_function(result_,down);
...
{
	int_* remainder = new int_[max_result];
	int_* quotient = new int_[max_result];
	int_* mass_ = new int_[max_result];
	int_* _mass = new int_[max_result];

	null_function(remainder, max_result);
	null_function(quotient, max_result);
	null_function(mass_, max_result);
	null_function(_mass, max_result);
